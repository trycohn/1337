...
3. **Backend логи**: `sudo journalctl -u 1337-backend -f`
=======
## 2025-01-22 - ✅ ИСПРАВЛЕНО: Конфликт портов EADDRINUSE

### 🚨 Исходная проблема:
- **Ошибка:** `Error: listen EADDRINUSE: address already in use :::3000`
- **Причина:** Конфликт между двумя серверами на одном порту
- **Влияние:** Периодические сбои запуска backend сервера

### 🔍 Диагностика проблемы:
- **Основной сервер:** `backend/server.js` (порт 3000)
- **Тестовый сервер:** `backend/simple-server.js` (тоже порт 3000!) 
- **PM2 процессы:** Возможные множественные запуски

### ✅ Решения:

#### 1. **Исправлен конфликт портов:**
- ✅ `simple-server.js`: Порт изменен с 3000 → 3001
- ✅ `server.js`: Улучшена обработка ошибок EADDRINUSE
- ✅ Добавлены подробные диагностические сообщения

#### 2. **Улучшена обработка ошибок в основном сервере:**
- ✅ Детальные сообщения о причинах ошибки
- ✅ Практические советы по решению
- ✅ Автоматическое завершение при критических ошибках

#### 3. **Создан диагностический скрипт:**
- ✅ `fix-port-conflicts.sh` - автоматическое решение конфликтов
- ✅ Проверка занятых портов через `netstat`
- ✅ Управление PM2 процессами
- ✅ Безопасная перезагрузка сервера

### 🔧 Команды для решения на VDS:

```bash
# 1. Сделать скрипт исполняемым
chmod +x /var/www/1337community.com/backend/fix-port-conflicts.sh

# 2. Запустить диагностику и исправление
./fix-port-conflicts.sh

# 3. Альтернативно - ручные команды:
pm2 stop all
...
### 📊 Результат:
- ✅ **Устранен конфликт портов** между серверами
- ✅ **Понятные ошибки** при проблемах с портами
- ✅ **Автоматическая диагностика** через bash скрипт
- ✅ **Предотвращение** будущих конфликтов

### 💡 Профилактика:
- Тестовый сервер теперь использует порт 3001
- Улучшенная обработка ошибок в production
- Автоматизированные скрипты для VDS обслуживания
>>>>>>> 8baea73 (fix)
