...
## ♻️ ПОСЛЕДНИЕ ИЗМЕНЕНИЯ (v4.31.0) - РЕЗУЛЬТАТЫ ТУРНИРОВ + ГЕЙМИФИКАЦИЯ

**Дата**: 23 октября 2025  
**Описание**: Система статистики турниров с MVP, геймификация с достижениями и наградами, переработанная вкладка результатов

### 🏆 Статистика турниров с MVP (v4.28.0)

**Возможности:**
- Автоматическое определение MVP турнира по формуле (без HLTV Rating)
- Панель лидеров турнира (7 категорий): MVP, Most Kills, Highest ADR, Best HS%, Best Accuracy, Clutch King, Total Money
- Автоматическое обновление статистики после каждого завершенного матча
- Агрегация данных из MatchZy webhook
- API endpoints для получения статистики и лидербордов

**Формула MVP (Формула B - без Rating):**
```
MVP_Points = (
...
) × min(matches_played, 5)   // Match weight
```

**Компоненты:**
- `TournamentStatsPanel.js` - блок со статистикой MVP и лидеров
- `TournamentStatsService.js` - бизнес-логика расчета
- `TournamentStatsRepository.js` - работа с БД
- Таблицы: `tournament_player_stats`, `tournament_achievements`

**Интеграция:**
- Автоматический вызов при сохранении результата матча
- Финализация турнира через `/api/tournaments/:id/stats/finalize`
- Отображение на вкладке "Результаты" (для завершенных турниров с лобби)

### 🎮 Геймификация турниров (v4.30.0)

**Achievement Badges:**
- Автоматическое создание badges за топ-3 в 8 категориях
- Визуальные значки с медалями (🥇🥈🥉)
- Отображение под каждым лидером в TournamentStatsPanel
- Хранение в `user_tournament_achievements`

**Leet Coins награды:**
- 👑 MVP: 500/250/100 Leet Coins
...
- **Максимум:** 1600 Leet Coins за турнир

**Global MVP Leaderboard:**
- Таблица `global_mvp_leaderboard` с автообновлением через триггер
- Формула: `(MVP×100) + (Gold×10) + (Silver×5) + (Bronze×2)`
- Страница `/leaderboard` (только для админов платформы)
- Компонент `GlobalMVPLeaderboard` с фильтрами и сортировкой
- Интеграция в профиль: вкладка "Достижения" с турнирными достижениями

**Компоненты:**
- `AchievementRewardsService.js` - логика начисления наград
- `AchievementsController.js` - API для достижений
- `AchievementBadge.js` - компонент значка с медалью
- `UserAchievementsPanel.js` - панель достижений в профиле
- `GlobalMVPLeaderboard.js` - глобальный рейтинг
- `GlobalLeaderboardPage.js` - отдельная страница рейтинга
- `AdminRoute.js` - защита маршрутов для админов
...
### 📊 Вкладка "Результаты" v2.0 - FACEIT Classic (v4.31.0)

**Двухколоночный layout для завершенных турниров:**
- **Левая колонка (40%):** Таблица мест команд (TeamStandingsTable)
- **Правая колонка (60%):** Статистика MVP и лидеры (TournamentStatsPanel)
- **Внизу (100%):** Улучшенная история матчей v2.0

**TeamStandingsTable - Квалификация команд:**
- Расчет мест через StandingsService для всех типов турниров
- Медали для топ-3 (🥇🥈🥉)
- Цифры для остальных мест (4, 5-8, 9-16, ...)
- Кликабельные аватары участников (макс 5 + индикатор +N)
- Корона для капитанов
- W:L статистика каждой команды
- Метка "Mix Team" для Mix турниров
...
**Single Elimination:**
- 1-2 места: из финала
- 3-4 места: из матча за 3 место ИЛИ проигравшие полуфиналов (если матча нет)
- 5+ места: группировка по раундам выбывания

**Double Elimination:**
- 1 место: победитель Grand Final
- 2 место: проигравший Grand Final
- 3 место: проигравший Loser Final
- 4+ места: группировка по Losers Bracket раундам
- Fallback для команд без placement

**Swiss:**
- 1-2 места: из финала
- Остальные: не ранжируются (Swiss не имеет bracket выбывания)

**Mix турниры (Full Mix, Classic Mix):**
- Используют `tournament_teams` (не participants!)
- Составы обновляются на фактические из последнего матча
- Метод `_updateMixTeamRosters()` берет данные из `matches.team1_players/team2_players`
- Отображается состав **на момент выбывания/победы** (важно для Full Mix!)

**История матчей v2.0 - Улучшенный дизайн:**
- Карточки с заголовком (#N матч + тип брекета)
- Аватары обеих команд + аватары всех участников (5 игроков)
- Список сыгранных карт со счетом
- Кнопка "Посмотреть матч →" (desktop) или кликабельная строка (mobile)
- Fallback на 0 для пустых значений счета карт

**Компоненты:**
- `StandingsService.js` - универсальный расчет мест для всех типов турниров
- `TeamStandingsTable.js` - таблица квалификации
- `TournamentResults.js` - двухколоночный layout
- Детальное логирование на каждом шаге расчета

**API Endpoints:**
```
GET /api/tournaments/:id/standings
  → Возвращает итоговую таблицу мест всех команд/участников
```

**Файлы:**
- `backend/services/tournament/StandingsService.js` (новый)
- `frontend/src/components/tournament/TeamStandingsTable.js` (новый)
- `frontend/src/components/tournament/TeamStandingsTable.css` (новый)
- `frontend/src/components/tournament/TournamentResults.js` (обновлен)
- `frontend/src/components/tournament/TournamentResults.css` (обновлен)
