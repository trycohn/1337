...
class BracketGenerationService {
    ...
    static async _getTournamentWithValidation(tournamentId, userId, client) {
        ...
        if (tournament.status !== 'active') {
            throw new Error('Турнирная сетка может быть сгенерирована только для активных турниров');
        }
    ...
    static async _updateTournamentAfterGeneration(tournament, generationResult, client) {
        ...
        await client.query(
            'UPDATE tournaments SET status = $1 WHERE id = $2',
            ['in_progress', tournament.id]
        );
        
        console.log(`📊 [BracketGenerationService] Турнир ${tournament.id} обновлен после генерации сетки`);
    }
