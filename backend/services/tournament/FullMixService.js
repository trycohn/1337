...
class FullMixService {
    ...
    static async confirmRoundRosters(tournamentId, roundNumber) {
        ...
        try {
            ...
            // 🆕 Проверяем статус подтверждения (разрешаем повторное подтверждение)
            const alreadyConfirmed = snapshot.snapshot?.meta?.rosters_confirmed;
            if (alreadyConfirmed) {
                console.log(`⚠️ Составы раунда ${roundNumber} уже были подтверждены ранее. Обновляем данные...`);
            }
