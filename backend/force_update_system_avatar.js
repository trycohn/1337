...
async function forceUpdateSystemAvatar() {
    try {
        ...
        // Дополнительно: принудительно обновляем timestamp чатов для сброса кэша
        try {
            await pool.query(`
                UPDATE chats 
                SET updated_at = NOW() 
                WHERE name = '1337community' AND type = 'system'
            `);
            console.log('✅ Обновлены timestamps системных чатов для сброса кэша');
        } catch (chatError) {
            console.log('ℹ️ Ошибка обновления timestamps чатов:', chatError.message);
        }
