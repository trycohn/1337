...
// Добавим базовые стили для wrapper и controls здесь же, если нет отдельного CSS
const styles = `
...
.bracket-title {
    position: absolute; /* Или sticky */
    top: 5px; /* Примерное позиционирование */
    left: 10px; /* Примерное позиционирование */
    background: rgba(240, 240, 240, 0.8); /* Полупрозрачный фон */
    padding: 2px 8px;
    border-radius: 4px;
    z-index: 5; /* Ниже контролов, но выше сетки */
    font-size: 1.1em;
    margin: 5px 0 15px 0; /* Отступы */
    text-align: center;
}

/* Возможно, нужно будет обновить стили для .bracket-grid, .round-column и т.д.
   чтобы они корректно работали внутри flex контейнера .bracket-renderer-content */
.bracket {
    display: flex; /* Чтобы заголовок был отдельно */
    flex-direction: column;
    align-items: center; /* Центрируем заголовок, если он внутри */
}
.bracket-grid {
    display: flex;
...
.placement-match, .grand-final {
     /* Убедимся, что эти блоки имеют нужную структуру */
     display: flex;
     flex-direction: column;
     align-items: center;
}
