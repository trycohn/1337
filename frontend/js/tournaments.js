...
router.get('/:id/participants', async (req, res) => {
  try {
      ...
      const participantsResult = await pool.query('SELECT * FROM tournament_participants WHERE tournament_id = $1', [tournamentId]);
...
router.post('/:id/participants', async (req, res) => {
  try {
      ...
      await pool.query(
          'INSERT INTO tournament_participants (tournament_id, name) VALUES ($1, $2)',
          [tournamentId, name]
      );
...
router.post('/:id/participants/batch', async (req, res) => {
  try {
      const tournamentId = req.params.id;
      const { participants } = req.body;

      const insertValues = participants.map(name => `(${tournamentId}, '${name}')`).join(',');
      await pool.query(`INSERT INTO tournament_participants (tournament_id, name) VALUES ${insertValues}`);

      res.json({ status: 'success', message: 'Участники добавлены' });
  } catch (err) {
      console.error('Ошибка добавления списка участников:', err);
      res.status(500).json({ status: 'error', message: err.message });
  }
});
